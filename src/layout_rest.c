...
clayout_rest *layout_rest_alloc()
{
    clayout_rest *dl;

    if ((dl = calloc(1, sizeof(clayout_rest))) == NULL)
    {
        errprintf("malloc(%ld) failed\n", (long)sizeof(clayout_rest));
        return NULL;
    }
...
int layout_rest_destroy(clayout_rest *dl)
{
    if (dl != NULL)
    {
        if (dl->disk)
...
int layout_rest_initialize(clayout_rest *dl, char *disklayout, char *devname)
{
...
int layout_rest_get_disk_specs(clayout_rest *dl, cdiskspecs *specs)
{
    ...
    {
        errprintf("serializer_getbykeys_xxx() failed\n");
        return -1;
    }

    format_size(((u64)specs->length)*((u64)specs->sectsize), specs->size, sizeof(specs->size), 'h');

    return 0;
}

int layout_rest_get_part_specs(clayout_rest *dl, cpartspecs *specs, int partnum)
{
    char partnumid[256];
...
int layout_rest_show_device(clayout_rest *dl, char *partdesc, int id)
{
    double gigabyte = 1024LL * 1024LL * 1024LL;
    cdiskspecs diskspecs;
    cpartspecs partspecs;
    int partcnt = 0;
    int i;

    if (layout_rest_get_disk_specs(dl, &diskspecs) != 0)
    {
        errprintf("layout_rest_get_disk_specs() failed\n");
        return -1;
    }

    msgprintf(MSG_FORCE, "\nid=%d: Original disk: [%s] [%s] [size: %s] [sectsize: %ld] [disklabel: %s]\n",
        id, diskspecs.path, diskspecs.model, diskspecs.size, (long)diskspecs.sectsize, diskspecs.disklabel);

    for (i = 1; partcnt <= diskspecs.partcount && i<=MAX_PARTS; i++)
    {
        if (layout_rest_get_part_specs(dl, &partspecs, i) == 0)
        {
            msgprintf(MSG_FORCE, " -part=%.3ld type=[%s] filesystem=[%s], flags=[%s]\n", 
                    (long)partspecs.num, partspecs.type, strlen(partspecs.fstype) ? (partspecs.fstype) : "none",
                    partspecs.flags);
            msgprintf(MSG_FORCE, "           start=%012.3f end=%012.3f size=%012.3f (GiB.MiB)\n", 
                    (float)(partspecs.start * diskspecs.sectsize / gigabyte), 
                    (float)(partspecs.end * diskspecs.sectsize / gigabyte),
                    (float)(partspecs.length * diskspecs.sectsize / gigabyte
                    ));
            msgprintf(MSG_FORCE, "           start=%.12lld end=%.12lld size=%.12lld (sectors)\n", 
                    (long long)partspecs.start, (long long)partspecs.end, (long long)partspecs.length);
            partcnt++;
        }
    }
...
int layout_rest_create_disk(clayout_rest *dl)
{
    PedDiskType *type;
    cdiskspecs diskspecs;
...
int layout_rest_analyse_partitions(clayout_rest *dl)
{
    cpartspecs partspecs;
    cdiskspecs diskspecs;
    u64 curendbyte;
...
int layout_rest_check_protection(clayout_rest *dl)
{
...
int layout_rest_check_conditions(clayout_rest *dl)
{
    cdiskspecs diskspecs;
    char buffer1[512];
    char buffer2[512];
...
int layout_rest_recreate_primary_partitions(clayout_rest *dl)
{
    ...
    if (layout_rest_get_disk_specs(dl, &diskspecs) != 0)
    {   errprintf("layout_rest_recreate_primary_partitions: layout_rest_get_part_specs() failed\n");
        return -1;
    }
    ...
    {
        part_type = 0;
        if ((layout_rest_get_part_specs(dl, &partspecs, i) == 0) && partutil_is_extended(&partspecs))
    ...
    for (i = 0; i < dl->primpart_count; i++)
    {
        ...
        {
            if ((layout_rest_get_part_specs(dl, &partspecs, j)==0) &&
        ...
        if (layout_rest_get_part_specs(dl, &partspecs, maxpartstart_id)!=0)
        {
            errprintf("layout_rest_get_part_specs() failed\n");
            return -1;
        }
...
int layout_rest_recreate_logical_partitions(clayout_rest *dl)
{
    ...
    {
        if ((layout_rest_get_part_specs(dl, &partspecs, i)==0) && partutil_is_logical(&partspecs))
        {
...
int layout_rest_set_partitions_attributes(clayout_rest *dl)
{
    PedPartition *part;
    cpartspecs partspecs;
    int i;
...
int layout_rest_commit_changes(clayout_rest *dl)
{
    msgprintf(MSG_VERB2, "layout_rest_rest_device(%s): model=[%s]\n", dl->devname, dl->dev->model);
...
int restpt(char *partdesc, int id, struct s_strdico *dicocmdline)
{
    ...
    if ((dl = layout_rest_alloc()) == NULL)
    {
        errprintf("layout_rest_alloc() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }
    
    if (layout_rest_initialize(dl, partdesc, destdisk) != 0)
    {
        msgprintf(MSG_STACK, "layout_rest_initialize() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }
    
    layout_rest_show_device(dl, partdesc, id);
    
    partutil_show_disk_specs(destdisk);
        
    if (layout_rest_create_disk(dl) != 0)
    {
        msgprintf(MSG_STACK, "layout_rest_create_disk() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }
    msgprintf(MSG_DEBUG2, "ped_disk_get_max_primary_partition_count(%s)=%d\n", dl->devname, dl->maxprimary);

    if (layout_rest_analyse_partitions(dl) != 0)
    {
        msgprintf(MSG_STACK, "layout_rest_analyse_partitions() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }

    if (layout_rest_check_protection(dl) != 0)
    {
        msgprintf(MSG_STACK, "layout_rest_check_protection() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }

    if (layout_rest_check_conditions(dl)!=0)
    {
        msgprintf(MSG_STACK, "layout_rest_check_conditions() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }
    ...
    if ((dl->primpart_count>0) && (layout_rest_recreate_primary_partitions(dl) != 0))
    {
        errprintf("layout_rest_recreate_primary_partitions() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }

    if ((dl->logicpart_count>0) && (layout_rest_recreate_logical_partitions(dl) != 0))
    {
        msgprintf(MSG_STACK, "layout_rest_recreate_logical_partitions() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }

    if (layout_rest_set_partitions_attributes(dl) != 0)
    {
        msgprintf(MSG_STACK, "layout_rest_set_partitions_flags() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }
    ...
    if (layout_rest_commit_changes(dl) != 0)
    {
        errprintf("layout_rest_commit_changes() failed\n");
        ret=-1;
        goto restpt_restore_device_cleanup;
    }
...
int showpt(char *partdesc, int id)
{
    ...
    if (layout_rest_initialize(dl, partdesc, "none")!=0)
    {
        msgprintf(MSG_STACK, "layout_rest_initialize() failed\n");
        ret=-1;
        goto showpt_cleanup;
